...
// © zeguela
//@version=4
strategy(title="[BOT] SLP 2H ZEGUELA DEMABOT", commission_value=0.063, commission_type=strategy.commission.percent, initial_capital=100, default_qty_value=90, default_qty_type=strategy.percent_of_equity, overlay=true, process_orders_on_close=true)
...
stopPerlong = input(5.0, title='Stop Loss Long %', type=input.float, group="Stop Loss & Take Profit Settings") / 100
stopPershort = input(9.0, title='Stop Loss Short %', type=input.float, group="Stop Loss & Take Profit Settings") / 100
take1Perlong = input(28.0, title='Take Profit Long % 1', type=input.float, group="Stop Loss & Take Profit Settings") / 100
take1Pershort = input(8.0, title='Take Profit Short % 1', type=input.float, group="Stop Loss & Take Profit Settings") / 100
...
//Alert configuration     
_alertMessageOpenLong="SLPFTX(SLP-PERP) { cancel(which=all); wait(3s); market(side=buy, amount=100%); wait(10s); limit(position=0, postOnly=false, offset=@" + tostring(longTPEntrada) + "); stopMarket(position=0, offset=@" + tostring(longStopEntrada) + ")}"
_alertMessageCloseLong="SLPFTX(SLP-PERP) { cancel(which=all); continue(if=positionLong); market(position=0);}"
_alertmessageExitLong="SLPFTX(SLP-PERP) { wait(15s); cancel(which=all);trailingLimit(position=0, minOffset=0.01%, maxOffset=0.03%, reduceOnly=true);}"

_alertMessageOpenShort="SLPFTX(SLP-PERP) {cancel(which=all); wait(3s); market(side=sell, amount=100%); wait(10s); limit(position=0, postOnly=false, offset=@" + tostring(shortTPEntrada) + "); stopMarket(position=0, offset=@" + tostring(shortStopEntrada) + ")}"
_alertMessageCloseShort="SLPFTX(SLP-PERP) { cancel(which=all); continue(if=positionShort); market(position=0);}"
_alertMessageExitShort="SLPFTX(SLP-PERP) { wait(15s); cancel(which=all);trailingLimit(position=0, minOffset=0.01%, maxOffset=0.03%, reduceOnly=true);}"
